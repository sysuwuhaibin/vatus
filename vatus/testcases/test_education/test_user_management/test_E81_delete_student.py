...
class DeleteStudent(unittest.TestCase):
    def setUp(self):
        self.driver = None
        self.base_url = settings.test_parameters.get("education_base_url")

    def test_E81_delete_student(self):
        ...
        for web_type in web_types:
            ...
            time.sleep(3)
            student_names = ['deletestudent01','deletestudent02','deletestudent03','deletestu04']
            for student_name in student_names:
                driver.find_element_by_id("create_user").click()
                time.sleep(2)
                driver.find_element_by_id("username").clear()
                driver.find_element_by_id("username").send_keys(student_name)
                driver.find_element_by_id("fullname").clear()
                driver.find_element_by_id("fullname").send_keys(student_name)
                driver.find_element_by_id("email").clear()
                driver.find_element_by_id("email").send_keys(student_name+"@vinzor.com")
            ...
            driver.find_element_by_css_selector("input.form-control.input-sm").send_keys("deletestudent")
            time.sleep(5)
            if web_type == 'firefox':
            ...
            ###########################################
            driver.find_element_by_css_selector("input.form-control.input-sm").clear()
            driver.find_element_by_css_selector("input.form-control.input-sm").send_keys("deletestu")
            time.sleep(5)
