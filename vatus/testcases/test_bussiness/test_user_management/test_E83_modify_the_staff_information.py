...
class ModifyStaffInfo(unittest.TestCase):
    def setUp(self):
        self.driver = None
        self.base_url = settings.test_parameters.get("bussiness_base_url")

    def test_E83_modify_staff_info(self):
        ...
        for web_type in web_types:
            ...
            driver.find_element_by_id("username").send_keys("staff01")
            driver.find_element_by_id("fullname").clear()
            driver.find_element_by_id("fullname").send_keys("staff01")
            driver.find_element_by_id("email").clear()
            driver.find_element_by_id("email").send_keys("staff01@vinzor.com")
            ...
            driver.find_element_by_css_selector("input.form-control.input-sm").send_keys("staff01")
            time.sleep(5)
            if web_type == 'firefox':
                driver.find_element_by_xpath("/html/body/div[2]/div[2]/div/div/div[2]/"
                                             "div[2]/div[2]/div/table/tbody/tr/td[5]/div/a[1]").click()
            elif web_type == 'chrome':
                element = driver.find_element_by_xpath("/html/body/div[2]/div[2]/div/div/div[2]/"
                                                       "div[2]/div[2]/div/table/tbody/tr/td[5]/div/a[1]")
            ...
            driver.find_element_by_id("email").clear()
            driver.find_element_by_id("email").send_keys("staff@example.com")
            driver.find_element_by_id("confirm_action").click()
            time.sleep(2)
            ...
            driver.find_element_by_id("confirm_action").click()
            time.sleep(3)
            self.assertEqual("用户 staff01 信息修改成功", driver.find_element_by_class_name("gritter-without-image").
